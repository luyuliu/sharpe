...
def rf_interp(smap_sub9,smap_sub1,train_1km,date,test_size=0.2):
    ...
    # coordinates should NOT be covariates (lowers error)
    x1_train_9km = training.iloc[:,:2]._values#lat,lon
    rf_aux_9km = training.iloc[:,4:]._values
    #rf_aux_9km = training.iloc[:,[0,1,4,5,6,7,8,9]]._values
    # sm_train_9km = training.iloc[gr,:2]._values
    sm_train_9km=training.iloc[:,2]._values
    # predict on 1km grid
    sm_1km = np.array(cov_all)
    for n in range(1,6):
        rf=RandomForestRegressor(n_estimators=(100*n),random_state=44)#changing the number of trees in the forest
        #rf=RandomForestRegressor(max_features=n,random_state=44)#max number of features considered for splitting a node
        #rf=RandomForestRegressor(max_depth=n,random_state=44)#max number of levels in each decision tree
        #rf=RandomForestRegressor(min_samples_split=n,random_state=44)#min number of data points placed in a node before the node is split
        #rf=RandomForestRegressor(min_samples_leaf=n,random_state=44)#min number of data points allowed in a leaf node
    ...
    mae_array=pd.DataFrame(mae_all)
    nlag = int(mae_array.loc[mae_array.iloc[:,1].idxmin()][0])
    print 'nlag is '+str(nlag)
    # predict 1 km grids with optimal nlag   
    m_rf=RandomForestRegressor(n_estimators=(100*nlag), random_state=44)#random_state is a seed for reproducibility
    m_rf.fit(rf_aux_9km, sm_train_9km)
    # predict on in situ points
    m_predictions=m_rf.predict(sm_1km)
    importances = list(m_rf.feature_importances_)# List of tuples with variable and importance
    annual_importances.append([importances])
    m_mae = mean_absolute_error(sm_point,m_predictions)
    print('Mean Absolute Error:', round(m_mae, 2))
    ...
    predic_1km = rf.predict(train_1km)
...
for q in range(182,192):
    for region in range(len(files_9km)):
        ...
        rf_pred_1km.to_csv(s_fn1_rf,index = False)#add filepath
